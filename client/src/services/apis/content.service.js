...
export const contentService = {
  ...
  async getCountsByChapterSlug(chapter_slug, isServer) {
    try {
      let result;
      if (isServer) {
        result = await HTTPClient.get(`${process.env.API_URL}/api/v1/content/chapter/${chapter_slug}/counts`);
      } else {
        result = await HTTPClient.get(`/api/v1/content/chapter/${chapter_slug}/counts`);
      }
      return result;
    } catch(error) {
      throw error;
